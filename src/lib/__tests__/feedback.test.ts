...
describe('Feedback System', () => {
  ...
  describe('generateFeedback', () => {
    ...
    test('handles duplicate digits correctly (Wordle-style)', () => {
      ...
      expect(feedback).toEqual([
        ...
        'wrong-position', // c=1 exists but wrong position (second 1)
  ...
  describe('calculateDigitTracker', () => {
    test('calculates digit status from multiple guesses', () => {
      const guess1: Guess = {
        matrix: { a: 1, b: 4, c: 7, d: 8 },
        vector: { e: 9, f: 6 },
        result: { g: 8, h: 9 },
        feedback: ['not-in-puzzle', 'not-in-puzzle', 'not-in-puzzle', 'not-in-puzzle', 'not-in-puzzle', 'not-in-puzzle'],
        timestamp: new Date()
      }

      const guess2: Guess = {
        matrix: { a: 2, b: 1, c: 3, d: 5 },
        vector: { e: 3, f: 2 },
        result: { g: 8, h: 9 },
        feedback: ['correct', 'wrong-position', 'wrong-position', 'not-in-puzzle', 'correct', 'correct'],
        timestamp: new Date()
      }
      ...
      expect(digitStatus['8']).toBe('not-in-puzzle')
      expect(digitStatus['9']).toBe('not-in-puzzle')
    })
    ...
    test('prioritizes correct over wrong-position', () => {
      const guess1: Guess = {
        ...
        vector: { e: 5, f: 6 },
        result: { g: 8, h: 9 },
        feedback: ['wrong-position', 'wrong-position', 'not-in-puzzle', 'not-in-puzzle', 'not-in-puzzle', 'not-in-puzzle'],
        timestamp: new Date()
      }
      ...
      const guess2: Guess = {
        ...
        vector: { e: 9, f: 5 },
        result: { g: 8, h: 9 },
        feedback: ['correct', 'correct', 'not-in-puzzle', 'not-in-puzzle', 'not-in-puzzle', 'not-in-puzzle'],
        timestamp: new Date()
      }
